...
public class FireflyAlgorithm
{
    ...
    public FireflyAlgorithm(List<Alternative> alternatives, List<Criterion> criteria, int maxIter, double alpha, double beta0, double gamma)
    {
            ...
            for (int i = 0; i < _alternatives.Count; i++)
            {
                for (int j = 0; j < _alternatives.Count; j++)
                {
                    if (_alternatives[i].FitnessValue < _alternatives[j].FitnessValue)
                    {
                        ...
                        _alternatives[i].FitnessValue += beta * (_alternatives[j].FitnessValue - _alternatives[i].FitnessValue)
